...
# ----------------------------------------------------------------------
# Name: DEF_PROFILE
# Definning the profile of each user
...
def edit_profile(request,user):
        ...
        if (request.user.username==user):
            ...
            for j in tp:
                if (tt == str(j.username)):
                    ...
                    print "Imprimimos las variables desc e imag"
                    print "0." + description + " " + path_image
            ...
            if request.method=="POST":
                ...
                if form.is_valid():
                    ...
                    tp = UserProfile.objects.all()
                    for u in tp:
                        print "recorro la lista tp: " + str(u.username)
                        print "tt es: " + str(tt)
                        if str(u.username) == str(tt):
                            print "son iguales"
                            if not (nickname==""):
                                nicki = User.objects.get(username=tt)
                                nicki.username = nickname
                                nicki.save()
    ...
    #                                print "8888888888888888888888888888888888888888888888888888888888"
                            ...
                            #        
                            #        print "3." + description + " " + path_image    
    ...
    #                            print "se han guardado los datos del perfil de usuario"
                            ...
                            print "1." + description + " " + path_image
                            ...
                            print "2." + description + " " + path_image
                            form = ProfileForm()
...
#            print "No se han encontrado ningun objecto con ese id"  
        ...
        if request.method=="GET":
            ...
            ctx = {'uni':uni, 'info':info, 'no_info': no_info, 'info_list':info_list, 'user_editor': user_editor_list.username, 'path_image':path_image, 'uniname':uniname.uni,  'uni_name':uni_name, 'type_user': g_type_user, 'username':request.user.username}
...
#        def_profile(request.user.username)
